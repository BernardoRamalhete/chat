export default function (canvasElement, logoText, colorPallet, spreadVelocity = 1) {
    //All heavy lifting was bone by Ana Tudor @ code pen
    //Source: https://codepen.io/thebabydino/pen/VwVXYzX
    const _C = canvasElement, 
			CT = _C.getContext('2d'), 
			DIR = ['x', 'y'], 
			TEXTBOX = { str: logoText }, 
			PALETTE = colorPallet, 
			POINTER = {},
			PARTICLES = [], 
			N = PALETTE.length - 1;
        ...
        move() {
            ...
            if(POINTER.x && POINTER.y) {
                ...
                * inversely proportional to distance squared */
                this.pv = this.f*Math.max(spreadVelocity, 1/Math.pow(pd, 1));
                this.ov = od ? this.f*Math.max(spreadVelocity, 1/Math.pow(od, 1)) : 0;
